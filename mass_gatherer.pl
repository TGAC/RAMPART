...
=head1 OPTIONS
  ...
  seq_info         Path to the sequence info tool.  Use only if sequenceinfo isn't already on the path.
  index            Prefix each line of output with the index of each scaffold file.  This is intended to be used instead of the kmer values which would be generated by mass.  Useful for the improver tool.
