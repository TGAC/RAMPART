...
public interface TaskExecutionService {
    ...
    /**
     * Execute a defined {@link uk.ac.tgac.rampart.conan.conanx.exec.task.ConanExternalTask}.  The task may be executed in the foreground or the background depending
     * on how the {@link ExecutionContext} is configured.
     * {@link ExecutionContext} and waits for it to complete before returning.
     * @param conanXProcess The {@link uk.ac.tgac.rampart.conan.conanx.exec.task.ConanExternalTask} to execute
     * @param executionContext The {@link ExecutionContext} within which to execute the {@link uk.ac.tgac.rampart.conan.conanx.exec.task.ConanExternalTask}
     ...
     */
    int execute(ConanExternalTask conanXProcess, ExecutionContext executionContext)
            throws InterruptedException, ProcessExecutionException;

    /**
     * Execute a command in the shell.  The task may be executed in the foreground or the background depending
     * on how the {@link ExecutionContext} is configured. This is used for simple shell commands that do not require any
     * specific task management.  E.g. for changing directory, or linking files.  For tasks that are non-trivial, i.e
     * tasks that do not complete within a couple of seconds it is recommended that the user creates a ConanExternalTask and
    ...
    /**
     * If a task or command (or set of processes or commands) was executed in the background, then this method can be
