 ...
 * Time: 17:51
 */
public class LSFFileProcessAdapter extends AbstractFileProcessAdapter {

    private static Logger log = LoggerFactory.getLogger(LSFFileProcessAdapter.class);

    private boolean commencedStdout = false;
    private boolean finishedStdout = false;

    public LSFFileProcessAdapter(String pathname, int monitoringPeriod) {
        super(pathname, monitoringPeriod);
    }

    public LSFFileProcessAdapter(String parent, String child, int monitoringPeriod) {
        super(parent, child, monitoringPeriod);
    }

    public LSFFileProcessAdapter(File parent, String child, int monitoringPeriod) {
        super(parent, child, monitoringPeriod);
    }

    public LSFFileProcessAdapter(URI uri, int monitoringPeriod) {
