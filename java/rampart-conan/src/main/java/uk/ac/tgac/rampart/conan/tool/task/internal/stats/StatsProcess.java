...
import uk.ac.tgac.rampart.conan.tool.task.external.r.RV2122Args;
import uk.ac.tgac.rampart.conan.tool.task.external.r.RV2122Task;
import uk.ac.tgac.rampart.conan.tool.proc.RampartProcess;
import uk.ac.tgac.rampart.conan.tool.proc.Stage;
import uk.ac.tgac.rampart.conan.tool.proc.util.RHelper;
...
public class StatsProcess implements ConanProcess, RampartProcess {
    ...
    @Override
    public void execute(ExecutionContext env) throws ProcessExecutionException, InterruptedException {

        // Assume we want to task all the FastA files in the directory specified by the user.
    ...
    protected void plot(File outputDir, File statsFile, ExecutionContext env) throws InterruptedException, ProcessExecutionException {
        ...
        // Create the Mass Plotter R task
        RV2122Task statsPlotterProcess = new RV2122Task(rArgs);

        // Execute the Mass Plotter R script.
        this.taskExecutionService.execute(statsPlotterProcess, env);
    }
}
