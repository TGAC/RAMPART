 ...
 **/
package uk.ac.tgac.rampart.conan.tool.proc.mass.single;
...
import uk.ac.tgac.rampart.conan.tool.proc.util.PerlHelper;
...
public class SingleMassProcess implements ConanProcess, RampartProcess {

    @Autowired
    private TaskExecutionService taskExecutionService;

    private SingleMassArgs args;
    ...
    public SingleMassArgs getArgs() {
        return args;
    }

    public void setArgs(SingleMassArgs args) {
        this.args = args;
    }
	...
	public void execute(ExecutionContext executionContext) throws IOException, ProcessExecutionException, InterruptedException, CommandExecutionException {
        ...
        if (executionContext.usingScheduler()) {
            schArgsBackup = executionContext.getScheduler().getArgs();
            schArgsCopy = executionContext.getScheduler().getArgs().copy();
		...
		for(int k = getFirstValidKmer(this.args.getKmin()); k <= this.args.getKmax(); k = nextKmer(k)) {
            ...
            if (schArgsCopy != null) {
                ...
                executionContext.getScheduler().setArgs(schArgsCopy);
            }

            // Create task
            this.taskExecutionService.execute(this.args.getAssembler(), executionContext);
        }
        ...
        if (executionContext.usingScheduler()) {
            executionContext.getScheduler().setArgs(schArgsBackup);
        }
	...
	protected void dispatchStatsJob(ExecutionContext env) throws InterruptedException, ProcessExecutionException, IOException, CommandExecutionException {
        ...
        // Create task
        this.taskExecutionService.execute(statCommands.toString(), env);
    ...
    public Collection<ConanParameter> getParameters() {
        return new SingleMassParams().getConanParameters();
    }
}
